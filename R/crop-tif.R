...
crop_worldpop_tif <- function (city = "accra", expand = 0.1)
{
    ...
    for (f in files)
    {
        zfiles <- utils::unzip (f, list = TRUE)$Name
        f1 <- zfiles [grep ("tif$", zfiles, ignore.case = TRUE)]
        utils::unzip (f, files = f1)
        ...
        utils::setTxtProgressBar (pb, count / n)
        count <- count + 1
    }
    close (pb)
}
...
crop_global_tif <- function (city = "bristol", expand = 0.1)
{
    ...
    zfiles <- utils::unzip (globes, list = TRUE)$Name
    f1 <- zfiles [grep ("tif$", zfiles, ignore.case = TRUE)]
    utils::unzip (globes, files = f1)
